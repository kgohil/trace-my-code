#!/usr/bin/env bash
# trace-my-code — pre-push doc-drift hook.
#
# Once per push: find changed files that live in a DOCUMENTED area (a dir with
# ARCHITECTURE.md / CLAUDE.md / DATA_FLOW.md, or a path an ADR's `governs:`
# lists). Then either auto-refresh the governing docs (rewrite mode) or just
# flag them (flag mode).
#
# Config (env or git config):
#   TRACE_MY_CODE_MODE = rewrite | flag      (default: flag — safe)
#   TRACE_MY_CODE_CLAUDE = claude            (the Claude CLI binary)
#
# Rewrite mode requires the Claude CLI. It edits ONLY stale sections (grounded in
# the diff), commits the result as its own visible commit, and ABORTS the push so
# that doc commit is included — review with `git show`, then push again.
set -euo pipefail

MODE="${TRACE_MY_CODE_MODE:-$(git config --get traceMyCode.mode || echo flag)}"
CLAUDE_BIN="${TRACE_MY_CODE_CLAUDE:-claude}"
SKILL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ROOT="$(git rev-parse --show-toplevel)"

# Range being pushed: compare against the upstream/main merge-base.
BASE="$(git merge-base HEAD "@{upstream}" 2>/dev/null || git merge-base HEAD origin/main 2>/dev/null || echo "")"
[ -z "$BASE" ] && exit 0
CHANGED="$(git diff --name-only "$BASE"..HEAD -- "$ROOT" | sed "s#^#$ROOT/#")"
[ -z "$CHANGED" ] && exit 0

# Only CODE changes signal drift — editing a doc isn't itself stale-making (and a
# PR that adds/updates docs shouldn't flag the very docs it touches). Drop the doc
# files (ARCHITECTURE/CLAUDE/DATA_FLOW.md and anything under docs/) from the triggers.
CHANGED="$(printf '%s\n' "$CHANGED" | grep -vE '/(ARCHITECTURE|CLAUDE|DATA_FLOW)\.md$|/docs/' || true)"
[ -z "$CHANGED" ] && exit 0

# A file is "documented" if an ancestor dir holds a doc, or an ADR governs it.
doc_for() {  # echo the governing doc(s) for a changed file, or nothing
  local f="$1" d; d="$(dirname "$f")"
  while [ "$d" != "$ROOT" ] && [ "$d" != "/" ]; do
    for name in ARCHITECTURE.md CLAUDE.md DATA_FLOW.md; do
      [ -f "$d/$name" ] && echo "$d/$name"
    done
    d="$(dirname "$d")"
  done
  # ADRs whose `governs:` frontmatter matches this path. Process substitution +
  # `|| true` so a no-match grep (no ADR has `governs:` yet) can't fail the
  # function under `set -e -o pipefail` and silently abort drift detection.
  while IFS= read -r adr; do
    [ -n "$adr" ] || continue
    while IFS= read -r p; do
      [ -n "$p" ] && [[ "$f" == *"$p"* ]] && echo "$adr"
    done < <(awk '/^governs:/{flag=1;next}/^[a-z]+:/{flag=0}flag&&/-/{gsub(/[- ]/,"");print}' "$adr" 2>/dev/null || true)
  done < <(grep -rliE "governs:" "$ROOT/docs/adrs" 2>/dev/null || true)
  return 0
}

DOCS="$(while IFS= read -r f; do doc_for "$f"; done <<< "$CHANGED" | sort -u)"
[ -z "$DOCS" ] && exit 0

echo "[trace-my-code] changed files touch documented areas. Governing docs:"
echo "$DOCS" | sed "s#^$ROOT/#  - #"

# Citation integrity (advisory): warn when a governing doc cites a symbol that no longer
# exists in the cited file — catches renames/removals, the highest-value drift, and
# sidesteps line-number rot. Citation format: `path/to/file.ext › symbolName`. Never fails
# the push by itself; it's a heads-up the doc may be stale.
check_citations() {
  local doc cite path sym file hits=0
  while IFS= read -r doc; do
    [ -f "$doc" ] || continue
    while IFS= read -r cite; do
      [ -n "$cite" ] || continue
      path="${cite%% › *}"; sym="${cite##* › }"
      path="${path#\`}"; sym="${sym%\`}"
      sym="${sym%%(*}"                                    # drop "(...)"
      sym="$(printf '%s' "$sym" | tr -cd '[:alnum:]_')"   # bare identifier
      [ -n "$path" ] && [ -n "$sym" ] || continue
      file="$ROOT/$path"
      if [ ! -f "$file" ]; then
        echo "  ⚠ $(basename "$doc"): cited file missing — $path"; hits=$((hits+1)); continue
      fi
      grep -qF "$sym" "$file" 2>/dev/null || {
        echo "  ⚠ $(basename "$doc"): symbol '$sym' not found in $path (renamed/removed?)"; hits=$((hits+1)); }
    done < <(grep -oE '`[^`]+ › [^`]+`' "$doc" 2>/dev/null || true)
  done < <(printf '%s\n' "$DOCS")
  [ "$hits" -gt 0 ] && echo "[trace-my-code] $hits stale citation(s) above — fix or refresh the doc."
  return 0
}
check_citations

if [ "$MODE" != "rewrite" ]; then
  echo "[trace-my-code] flag mode — review the docs above (set TRACE_MY_CODE_MODE=rewrite to auto-refresh)."
  exit 0
fi

if ! command -v "$CLAUDE_BIN" >/dev/null 2>&1; then
  echo "[trace-my-code] rewrite mode but '$CLAUDE_BIN' not found — falling back to flag. Install the Claude CLI or set TRACE_MY_CODE_CLAUDE."
  exit 0
fi

PROMPT="Invoke the trace-my-code skill in Mode B (auto-update on drift), following references/auto-update-contract.md EXACTLY.
Changed files in this push:
$CHANGED
Governing docs to refresh (edit ONLY stale sections, grounded in the actual diff; preserve everything still accurate; flag unverifiable changes with an HTML comment instead of inventing):
$DOCS
Read the diff ($BASE..HEAD) for the changed files before editing. Make surgical edits only. If nothing material changed, make no edits."

echo "[trace-my-code] rewrite mode — refreshing docs via $CLAUDE_BIN ..."
( cd "$ROOT" && "$CLAUDE_BIN" -p "$PROMPT" --allowedTools "Read,Edit,Bash,Grep,Glob" ) || {
  echo "[trace-my-code] rewrite failed — push continues, docs unchanged."; exit 0; }

mapfile -t _doc_paths <<< "$DOCS"
if ! git -C "$ROOT" diff --quiet -- "$ROOT/docs" "${_doc_paths[@]}"; then
  git -C "$ROOT" add docs "${_doc_paths[@]}"
  git -C "$ROOT" commit -m "docs: auto-refresh architecture docs for changed areas

Generated by trace-my-code on pre-push. Grounded in the pushed diff;
review with \`git show HEAD\`." >/dev/null
  echo "[trace-my-code] doc-refresh commit added: $(git -C "$ROOT" rev-parse --short HEAD)"
  echo "[trace-my-code] review with 'git show HEAD', then push again to include it."
  exit 1   # abort this push so the doc commit is part of the next one
fi
echo "[trace-my-code] no doc changes needed."
exit 0
